//* FIXME: inadequate filename. Move to ..... ?
...
function getFocusedTerminalId ( activeEl = window.focusLaterElements[ 0 ] || document.activeElement, returnElement = false ) {
  if ( activeEl.tagName.toLowerCase() == 'body' )
    return false;

  const thisContainer = activeEl.closest( '.lm_item_container .lm_content' );
  return returnElement ? thisContainer : thisContainer.id;
}
